...
public class InsightARManager  : MonoBehaviour
{
    ...
    private void updatePositionLabel(Touch touch)
    {
        if (labelPosition == null)
        {
            GameObject obj = GameObject.Find("Canvas/LabelPosition");  
            if (obj != null)
            {
                labelPosition = obj.GetComponent<Text>();
                if (labelPosition == null)
                {
                    return;
                }
        ...
        labelPosition.text = "Position:" + m_ARInterface.GetHitTestResult(touch).position.ToString("f3");
    }
